...
<task type="auto">
  ...
  <action>Run `pnpm add date-fns@^4.1.0 @date-fns/tz@^1.2.0 react-simple-maps@^3.0.0 topojson-client@^3.1.0 d3-geo@^3.1.1` (runtime). Then `pnpm add -D @lhci/cli@^0.14.0 @axe-core/playwright@^4.10.0 @types/react-simple-maps @types/topojson-client @types/d3-geo`. Do NOT add `@vercel/speed-insights`, `web-vitals`, or any analytics package — anti-feature per CLAUDE.md. After install, run `pnpm typecheck` (i.e. `pnpm exec tsc --noEmit` — match the project's existing typecheck invocation; if absent add `"typecheck": "tsc --noEmit"` script) to confirm no type breakage. Commit lockfile.</action>
  <verify>
...
<task type="auto">
  ...
  <action>
    ...
    Separately resolve the br-atlas source: `npm view @carolinabigonha/br-atlas` — if 404, vendor `states.json` from https://github.com/carolinabigonha/br-atlas raw under `src/lib/geo/data/states.json` and document SHA. If published, install at pinned version.
    Write `05-01-spike-results.md` with: (a) PASS/PIVOT verdict on react-simple-maps SSR, (b) br-atlas decision (npm package vs vendored file + SHA), (c) if PIVOT, the fallback approach (precomputed `<path>` array via `d3-geo` `geoPath` server-side; downstream map plan switches to that). Delete `src/app/__spike/` and `scripts/spike/`.
  </action>
  <verify>
