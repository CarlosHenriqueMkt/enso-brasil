 ...
 *   5. Compose snapshot via risk engine: calculateRiskLevel + applyStaleness +
 *      generateExplanation per UF, sourced from active alerts in DB.
...
import { sql, and, or, gt, isNull } from "drizzle-orm";
...
// NOTE: messages import removed — riskReason now generated by generateExplanation.
import { calculateRiskLevel } from "@/lib/risk/calculate";
import { applyStaleness, FORMULA_VERSION } from "@/lib/risk/snapshot";
import { generateExplanation } from "@/lib/risk/explanation";
import type { SourcesHealthRow } from "@/lib/risk/types";
...
export async function POST(req: Request) {
  ...
  // Step 5a: query active alerts from DB (RISK-06: valid_until > now, or
  // (valid_until IS NULL AND fetched_at > now - 24h)). Group by state_uf so
  // the composer sees cumulative state, not just this-tick payloads.
  const composerNow = new Date();
  const twentyFourHoursAgo = new Date(composerNow.getTime() - 24 * 60 * 60 * 1000);
  const activeRows = await db
    .select()
    .from(alerts)
    .where(
      or(
        gt(alerts.validUntil, composerNow),
        and(isNull(alerts.validUntil), gt(alerts.fetchedAt, twentyFourHoursAgo)),
      ),
    );

  const activeAlertsByUF = new Map<string, Alert[]>();
  for (const r of activeRows) {
    const a: Alert = {
      source_key: r.sourceKey,
      hazard_kind: r.hazardKind as Alert["hazard_kind"],
      state_uf: r.stateUf as Alert["state_uf"],
      severity: r.severity as Alert["severity"],
      headline: r.headline,
      body: r.body ?? undefined,
      source_url: r.sourceUrl ?? undefined,
      fetched_at: r.fetchedAt.toISOString(),
      valid_from: r.validFrom ? r.validFrom.toISOString() : undefined,
      valid_until: r.validUntil ? r.validUntil.toISOString() : undefined,
      payload_hash: r.payloadHash,
      raw: r.raw,
    };
    const arr = activeAlertsByUF.get(a.state_uf) ?? [];
    arr.push(a);
    activeAlertsByUF.set(a.state_uf, arr);
  }

  // Step 5b: pull current sources_health into the staleness-engine shape.
  const healthRows = await db.select().from(sourcesHealth);
  const healthShape: SourcesHealthRow[] = healthRows.map((h) => ({
    source_key: h.sourceKey,
    last_successful_fetch: h.lastSuccessAt ? h.lastSuccessAt.toISOString() : null,
  }));

  // Step 5c: compose snapshot — risk engine + staleness override + PT-BR explanation.
  const curr: StateSnapshot[] = UF27.map((uf) => {
    const ufAlerts = activeAlertsByUF.get(uf) ?? [];
    const lastFetch = ufAlerts.reduce<string | null>(
      (acc, a) => (!acc || a.fetched_at > acc ? a.fetched_at : acc),
      null,
    );
    const baseLevel = calculateRiskLevel(ufAlerts, composerNow);
    const risk = applyStaleness(baseLevel, healthShape, composerNow);
    const riskReason = generateExplanation(risk, ufAlerts);
    return {
      uf,
      risk,
      riskReason,
  ...
  await db
    ...
    .values({
      snapshotKey: "current",
      body: curr,
      computedAt: composerNow,
      formulaVersion: FORMULA_VERSION,
    })
    .onConflictDoUpdate({
      target: snapshotCache.snapshotKey,
      set: { body: curr, computedAt: composerNow, formulaVersion: FORMULA_VERSION },
    });
